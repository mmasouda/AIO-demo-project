...
export const Dropdown = ({ title, options, onClick }: { title: string; options: string[]; onClick: (value: SetStateAction<string>) => void }) => {
    ...
    return (
        <div className="relative ml-4 inline-block text-left">
            <div>
                <button style={{ backgroundColor: "#303238" }} onClick={() => setOpen(!open)} type="button" className="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold shadow-xs ring-1 ring-gray-300 ring-inset" id="menu-button" aria-expanded="true" aria-haspopup="true">
                    {title}
